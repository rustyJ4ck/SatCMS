...
class image_extrafs_field_item extends extrafs_field_item {
    ...
    */
    function create_html_control() {             
        ...
        sprintf(
        <<<MECK
        <a href="javascript:;" onclick="$('#{$uid}').parent().append('<input type=hidden name=_efs[%s][%s] value=remove /><div class=help forered>Файл помечен к удалению</div>');$('#{$uid}').remove();$(this).hide();">Удалить</a>
MECK
            , ($this->get_group() ? $this->_group->name : '@fixme@')
            , $this->name
        ))
