 ...
 * 
 * @version 1.x $Id: console.php,v 1.3 2010/07/21 17:57:16 surg30n Exp $
 */
...
class Debug_HackerConsole_Main
{
    ...
    function attachToHtml($page)
    {
        ...
        if ($this->disable) return $page;

        $js = implode("", file(dirname(__FILE__).'/console.js'));
    ...
    function out($v, $group, $color=null) {
        $this->_out($v, 'console', $color);
    }

    function _out($v, $group="message", $color=null, $tip=null)
    {
    ...
    function toPre($text, $tabSize=null)
    {
        ...
        // Expand tabulators.
        if ($tabSize === null) {
            if (isset(core::$console)) 
                $tabSize = core::$console->TAB_SIZE;
            else 
                $tabSize = 4;
        }
    ...
    function debug_backtrace_smart($ignoresRe=null, $returnCaller=false)
    {
        ...
        for ($i=0, $n=count($trace); $i<$n; $i++) {
            ...
            // Skip myself frame.            
            if (++$framesSeen < (2 + self::$frame_skip)) continue;
