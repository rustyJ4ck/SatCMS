...
class mysql_db extends dbal
{
    ...
    private $_disabled_set_names = false;
    ...
    */
    function query($query = "", $transaction = FALSE) {
        return $this->sql_query($query, $transaction);
    }
    ...
    * @param mixed $transaction
    */
    function fetch_all($query = "", $transaction = FALSE) {
        $qid = $this->sql_query($query, $transaction);
        $result = $this->sql_fetchrowset($qid);
        return $result;
    }    
    ...
    */    
	function sql_query($query, $options = null)
	{
		...
		if ($this->query_result) {
            ...
            if (core::get_instance()->get_cfg_var('debug_sql') >= 100) {
 ...
 // cache num rows
    ...
    * Fetch rowset
    */
    function fetch_rowset($qid = 0) {
        return $this->sql_fetchrowset($qid);
    }
    ...
    */
    ...
    function free_result($qid = 0) {
        return $this->sql_freeresult($qid);
    }    
	function sql_freeresult($query_id = 0)
	{
        // false positive
        if ($query_id === false) return;
